...
function getOrderObjectByOrderId(args) {
	...
	return new Promise((resolve, reject) => {
		...
		PPOrder.findOne(searchQuery, null, { lean: true }, function (err, record) {
			if (record) {
				console.log(util.format('RECORD LOCATED FOR ORDER ID `%s`...', orderId));
			}
			resolve(record);
...
function getRecentWebhooks(args) {

	const { limit } = args;

	return new Promise((resolve, reject) => {

		PPWebhook.find({}, null, { sort: { 'INSERTION_DATE' : -1 }, limit, lean: true }, function(err, webhooks) {
		  resolve(webhooks);
		});
