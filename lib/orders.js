...
function createAccessToken(args) {
	return new Promise((resolve, reject) => {
		...
		const { environment, clientType, customClientId, customClientSecret, scheme, accessToken } = args;

		// If existing access token passed in, return it back and do not retrieve a new one
		if (accessToken) {
...
function determineClientId(args) {
	return new Promise((resolve, reject) => {
	    ...
	    if (!_.isEmpty(customClientId)) {
			...
			// Verify that scheme is not undefined, else default to generic default clientId and clientSecret
			if (scheme) {
