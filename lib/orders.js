...
function determineClientId(args) {
	return new Promise((resolve, reject) => {
	    ...
	    if (!_.isEmpty(customClientId)) {
			...
			if (scheme && puiUtils.isPUI(scheme)) {
				resolve({
					clientId: process.env[`PP_PUI_${environment}_CLIENT_ID_${webHookClientTag}`],
					clientSecret: process.env[`PP_PUI_${environment}_CLIENT_SECRET_${webHookClientTag}`]
				  });
			} else {
				resolve({
