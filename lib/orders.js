		...
		function runRequest() {
			...
			constructCreateOrderPayload(args).then((payload) => {
				...
				let ppOrder = new PPOrder({
					...
					CLIENT_ID:  process.env.PP_SANDBOX_CLIENT_ID_PRIMARY,
					PAYMENT_SCHEME: args.scheme,
					BUYER_NAME: args.name,
					BUYER_EMAIL: args.emailAddress,
					BUYER_COUNTRY: args.countryCode,
					AMOUNT: args.amount,
		...
		constructConfirmPaymentSourcePayload(args).then((payload) => {
			...
			dbUtils.getOrderByOrderId({ orderId }).then((record) => {
				...
				if (record) {
					...
					//   "links": [
					//     {
					//       "href": "https://api.sandbox.paypal.com/v2/checkout/orders/9N578587PB332660R",
					//       "rel": "self",
					//       "method": "GET"
					//     },
					//     {
					//       "href": util.format('%s?token=%s', process.env.PP_MOCK_APPROVAL_URL, orderId),
					//       "rel": "approve",
					//       "method": "GET"
					//     }
					//   ],
					...
					//   "debug_id": "c450e6f44463c",
					//   "links": [
					//     {
					//       "href": "https://developer.paypal.com/docs/api/orders/v2/#error-COUNTRY_NOT_SUPPORTED_BY_PAYMENT_SOURCE",
					...
					// record.CONFIRM_PAYMENT_SOURCE_API.CORRELATION_ID = util.format('MOCK%s', orderId);
...
function createAccessToken(args) {
	return new Promise((resolve, reject) => {
		...
		const options = {
			auth: {
				user: environment === 'LIVE' ? process.env.PP_LIVE_CLIENT_ID_PRIMARY : process.env.PP_SANDBOX_CLIENT_ID_PRIMARY,
				pass: environment === 'LIVE' ? process.env.PP_LIVE_CLIENT_SECRET_PRIMARY : process.env.PP_SANDBOX_CLIENT_SECRET_PRIMARY
			},
			method: 'POST',
	...
	return new Promise((resolve, reject) => {
		...
		dbUtils.getOrderByOrderId({ orderId }).then((record) => {
			...
			if (record) {
				...
				//             "links": [
				//               {
				//                 "href": "https://api.sandbox.paypal.com/v2/payments/captures/3TY926042S008591T",
				//                 "rel": "self",
				//                 "method": "GET"
				//               },
				//               {
				//                 "href": "https://api.sandbox.paypal.com/v2/payments/captures/3TY926042S008591T/refund",
				//                 "rel": "refund",
				//                 "method": "POST"
				//               },	
				//               {
				...
				// 	CORRELATION_ID: util.format('MOCK%s', orderId)
				// };
...
function constructCreateOrderPayload(args) {
	return new Promise((resolve, reject) => {
		...
		const { name, emailAddress, phoneNumber, returnUrl, cancelUrl, currency, amount, shippingPreference } = args;
...
function constructConfirmPaymentSourcePayload(args) {
	return new Promise((resolve, reject) => {
		...
		const { returnUrl, cancelUrl, scheme, name, emailAddress, countryCode, bic } = args;
