	...
	return new Promise((resolve, reject) => {
		...
		function runRequest() {
			...
			Promise.all([constructCreateOrderPayload(args),
						 determineClientId({ environment, clientType, customClientId, customClientSecret })])
...
function determineClientId(args) {
	return new Promise((resolve, reject) => {
	    ...
	    } else if (environment === 'LIVE' && ['oxxo', 'boletobancario', 'poli'].some(val => val === scheme)) {
			switch (scheme) {
				case 'oxxo':
					resolve({
						clientId: process.env[`PP_${environment}_CLIENT_ID_OXXO`],
						clientSecret: process.env[`PP_${environment}_CLIENT_SECRET_OXXO`]
					  });
					break;
				case 'boletobancario': 
					resolve({
						clientId: process.env[`PP_${environment}_CLIENT_ID_BOLETO`],
						clientSecret: process.env[`PP_${environment}_CLIENT_SECRET_BOLETO`]
					});
					break;
				case 'poli':
					resolve({
						clientId: process.env[`PP_${environment}_CLIENT_ID_POLI`],
						clientSecret: process.env[`PP_${environment}_CLIENT_SECRET_POLI`]
					});
					break;
			}
		} else {
