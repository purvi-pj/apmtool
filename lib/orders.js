...
function createOrder(args) {
    ...
    isNonInstant = ['oxxo', 'multibanco', 'boletobancario'].some(apm => apm === scheme);
...
function constructConfirmPaymentSourcePayload(args) {
	return new Promise((resolve, reject) => {
		...
		let { returnUrl, cancelUrl, scheme, name, emailAddress, countryCode, bic, expiresInDays } = args;
