	...
	return new Promise((resolve, reject) => {
		...
		function runRequest() {
			...
			Promise.all([constructCreateOrderPayload(args),
						 determineClientId({ environment, clientType })])
...
function createAccessToken(args) {
	return new Promise((resolve, reject) => {
		...
		const { environment, clientType } = args;
...
function determineClientId(args) {
	return new Promise((resolve, reject) => {
		...
		const IS_LIVE = environment == 'LIVE' ? true : false;

		const IS_WEBHOOK_CLIENT = clientType == 'WEBHOOK_CLIENT' ? true : false;

		if (IS_LIVE) {
			if (IS_WEBHOOK_CLIENT) {
				resolve({ clientId: process.env.PP_LIVE_CLIENT_ID_PRIMARY, clientSecret: process.env.PP_LIVE_CLIENT_SECRET_PRIMARY });
			} else {
				resolve({ clientId: process.env.PP_LIVE_CLIENT_ID_NO_WEBHOOK, clientSecret: process.env.PP_LIVE_CLIENT_SECRET_NO_WEBHOOK });
			}
		} else {
			if (IS_WEBHOOK_CLIENT) {
				resolve({ clientId: process.env.PP_SANDBOX_CLIENT_ID_PRIMARY, clientSecret: process.env.PP_SANDBOX_CLIENT_SECRET_PRIMARY });
			} else {
				resolve({ clientId: process.env.PP_SANDBOX_CLIENT_ID_NO_WEBHOOK, clientSecret: process.env.PP_SANDBOX_CLIENT_SECRET_NO_WEBHOOK });
			}
		}
